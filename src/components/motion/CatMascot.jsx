 ...
 * - Outlines and pupils are locked to a fixed dark color (#0B0B0C) so they remain
 *   fully detailed and visible on both light and dark backgrounds.
...
export default function CatMascot() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      // Normalized screen coordinates [-1, 1] relative to center
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ellipse cx="60" cy="86" rx="30" ry="20" fill={offWhite} stroke={darkInk} strokeWidth="2.5" />
        ...
        <circle cx="60" cy="56" r="30" fill={offWhite} stroke={darkInk} strokeWidth="2.5" />
        ...
        {mascotState === "sleeping" ? (
          <>
            ...
            <path d="M41 55 Q47 59 53 55" stroke={darkInk} strokeWidth="2.2" fill="none" strokeLinecap="round" />
            {/* Sleeping right eye curve */}
            <path d="M67 55 Q73 59 79 55" stroke={darkInk} strokeWidth="2.2" fill="none" strokeLinecap="round" />
          </>
        ) : activeSection === "contact" ? (
          <>
            {/* Winking Eyes (Left open, Right winking curve) */}
            <circle cx="48" cy="54" r="7" fill="#FFFFFF" stroke={darkInk} strokeWidth="1.8" />
            <motion.g style={enableHeavyFX ? { x: pupilX, y: pupilY } : undefined}>
              <circle cx="48" cy="54" r="3.5" fill={darkInk} />
              <circle cx="50" cy="52" r="1.1" fill="#FFFFFF" />
            </motion.g>
            {/* Right eye winking Happy curve */}
            <path d="M67 54 Q73 59 79 54" stroke={darkInk} strokeWidth="2.2" fill="none" strokeLinecap="round" />
          </>
        ) : activeSection === "about" ? (
          <>
            {/* Heart Eyes whites */}
            <circle cx="48" cy="54" r="7" fill="#FFFFFF" stroke={darkInk} strokeWidth="1.8" />
            <circle cx="72" cy="54" r="7" fill="#FFFFFF" stroke={darkInk} strokeWidth="1.8" />
          ...
          <>
            ...
            <path d="M 48 48 Q 48 54 42 54 Q 48 54 48 60 Q 48 54 54 54 Q 48 54 48 48 Z" fill="#FFB8E6" stroke={darkInk} strokeWidth="0.8" />
            <path d="M 72 48 Q 72 54 66 54 Q 72 54 72 60 Q 72 54 78 54 Q 72 54 72 48 Z" fill="#FFB8E6" stroke={darkInk} strokeWidth="0.8" />
          </>
        ) : (
          <>
            ...
            <motion.g style={enableHeavyFX ? { x: pupilX, y: pupilY } : undefined}>
              <circle cx="72" cy="54" r="3.5" fill={darkInk} />
        ...
        {activeSection === "process" && mascotState !== "sleeping" && (
          <g style={{ zIndex: 12 }}>
            <circle cx="48" cy="54" r="9.5" fill="none" stroke={darkInk} strokeWidth="1.5" />
            <circle cx="72" cy="54" r="9.5" fill="none" stroke={darkInk} strokeWidth="1.5" />
            <path d="M57.5 54 Q60 51.5 62.5 54" stroke={darkInk} strokeWidth="1.5" fill="none" />
            <path d="M38.5 54 L32 52" stroke={darkInk} strokeWidth="1.2" fill="none" />
            <path d="M81.5 54 L88 52" stroke={darkInk} strokeWidth="1.2" fill="none" />
          </g>
        )}
        ...
        {activeSection === "work" && mascotState !== "sleeping" && (
          <g style={{ zIndex: 12 }}>
            ...
            <path d="M36 49 H58 V57 C58 61.5, 53.5 63, 47 63 C40.5 63, 36 61.5, 36 57 Z" fill={darkInk} />
            <path d="M62 49 H84 V57 C84 61.5, 79.5 63, 73 63 C66.5 63, 62 61.5, 62 57 Z" fill={darkInk} />
            {/* Frame top bar */}
            <rect x="34" y="47" width="52" height="3.2" rx="1.5" fill={darkInk} />
        ...
        <circle cx="56" cy="62" r="4.5" fill={offWhite} stroke={darkInk} strokeWidth="1.5" />
        <circle cx="64" cy="62" r="4.5" fill={offWhite} stroke={darkInk} strokeWidth="1.5" />

        {/* Pink Nose */}
        <path d="M60 59 C59.5 59, 58.5 59.5 58.5 60.5 C58.5 61.5, 60 62.5, 60 62.5 C60 62.5, 61.5 61.5, 61.5 60.5 C61.5 59.5, 60.5 59, 60 59 Z" fill="#FF88A8" stroke={darkInk} strokeWidth="0.8" />

        {/* Mouth expression (drops open in surprise when scrolling) */}
        {activeSection === "contact" && mascotState !== "sleeping" ? (
          <ellipse cx="60" cy="66" rx="2" ry="2.8" fill="#FF88A8" stroke={darkInk} strokeWidth="1.2" />
        ) : (
          <path d="M56 62 Q58 64 60 62 Q62 64 64 62" stroke={darkInk} strokeWidth="1.8" fill="none" strokeLinecap="round" />
        )}

        {/* Playful pink tongue sticking out in "SKILLS" section */}
        {activeSection === "skills" && mascotState !== "sleeping" && (
          <path d="M58 62 C58 62, 60 68, 62 68 C64 68, 64 62, 64 62 Z" fill="#FF88A8" stroke={darkInk} strokeWidth="1" />
        )}

        {/* Paws resting on the bottom edge */}
        <circle cx="42" cy="85" r="7.5" fill={offWhite} stroke={darkInk} strokeWidth="2" />
        <circle cx="78" cy="85" r="7.5" fill={offWhite} stroke={darkInk} strokeWidth="2" />
        {/* Paw pad tick lines */}
        <path d="M40 82 L40 85 M44 82 L44 85" stroke={darkInk} strokeWidth="1.2" strokeLinecap="round" />
        <path d="M76 82 L76 85 M80 82 L80 85" stroke={darkInk} strokeWidth="1.2" strokeLinecap="round" />

        {/* Whiskers */}
        <g stroke={darkInk} strokeWidth="1.2" opacity="0.6" strokeLinecap="round">
      ...
      {isPawing && (
        ...
        >
          ...
          <ellipse cx="20" cy="20" rx="12" ry="9" fill={offWhite} stroke={darkInk} strokeWidth="1.8" />
          <circle cx="10" cy="14" r="3" fill={pinkAccent} stroke={darkInk} strokeWidth="1" />
          <circle cx="16" cy="9" r="3" fill={pinkAccent} stroke={darkInk} strokeWidth="1" />
          <circle cx="24" cy="9" r="3" fill={pinkAccent} stroke={darkInk} strokeWidth="1" />
          <circle cx="30" cy="14" r="3" fill={pinkAccent} stroke={darkInk} strokeWidth="1" />
        </motion.svg>
      )}
    </motion.button>
  );
